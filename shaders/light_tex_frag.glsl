...
uniform float matSh;
uniform vec3 srcDiff, srcSpec, srcAmbi; // 광원
// srcDiff = 난반사됐을 때 보이는 전체적인 빛
// srcSpec = 정반사됐을 때 나타나는 반짝이는 하이라이트 부분
...
void main()
{
    ...
    // Phong Model -> 이 모델은 수학적 계산했을 때 큐브의 어두운 부분에 빛의 하이라이트 부분이 보인다. 
